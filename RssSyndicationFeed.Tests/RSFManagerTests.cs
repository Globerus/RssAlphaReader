...
namespace RssSyndicationFeed.Tests
{
    ...
    public class RSFManagerTests
    {
        ...
        {
            var xml = Resources.BNT;
            var xReader = GenerateReaderFromXML(xml);
            var feed = RssSyndicationFeedManager.Load(xReader);

            Assert.IsNotNull(feed);

            var extension = feed.Extensions.FirstOrDefault().Context as RssSyndicationFeedContext;
            Assert.AreEqual("http://news.bnt.bg/bg/rss/news.xml", extension.Link.FirstOrDefault()?.Href);
            Assert.AreEqual("self", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Rel);
            Assert.AreEqual("application/rss+xml", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Type);
        ...
        {
            var xml = Resources.TrafficNewsBG;
            var xReader = GenerateReaderFromXML(xml);
            var feed = RssSyndicationFeedManager.Load(xReader);
        ...
        {
            var xml = Resources.VestiBG;
            var xReader = GenerateReaderFromXML(xml);
            var feed = RssSyndicationFeedManager.Load(xReader);

            Assert.IsNotNull(feed);

            Assert.AreEqual("https://www.vesti.bg/rss", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Href);
            Assert.AreEqual("self", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Rel);
            Assert.AreEqual("application/rss+xml", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Type);
        ...
        public void RSFManagerLoadCNN()
        {
            ...
            var link1 = (extension.Context as RssSyndicationFeedContext).Link.Where(l => l.Rel == "self").FirstOrDefault();
            var link2 = (extension.Context as RssSyndicationFeedContext).Link.Where(l => l.Rel == "hub").FirstOrDefault();
        ...
        [TestMethod]
        public void RSFManagerLoadCNNWorld()
        {
            var xml = Resources.CNNWorld;
            var xReader = GenerateReaderFromXML(xml);
            var feed = RssSyndicationFeedManager.Load(xReader);
        ...
        public void RSFManagerLoadSportalBG()
        {
            ...
            Assert.AreEqual("http://www.sportal.bg/uploads/rss_category_0.xml", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Href);
            Assert.AreEqual("self", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Rel);
            Assert.AreEqual("application/rss+xml", ((RssSyndicationFeedContext)feed.Extensions.FirstOrDefault()?.Context).Link.FirstOrDefault()?.Type);
